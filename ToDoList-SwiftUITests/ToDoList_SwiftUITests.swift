...
final class ToDoList_SwiftUITests: XCTestCase {
    ...
    func test_AddToDo() async {
        let beforeCount = await MainActor.run { viewModel.sortedTodo.count }
        await MainActor.run { viewModel.addToDo(text: "할 일") }
        let afterCount = await MainActor.run { viewModel.sortedTodo.count }
    ...
    func test_setAllToDoData() {
        let todoDummyData = viewModel.sortedTodo
    ...
    func test_onOffToDo() {
        let todoDummyDataID = viewModel.sortedTodo[0].id
        viewModel.onOffToDo(id: todoDummyDataID)
        XCTAssertTrue(viewModel.sortedTodo[0].isCompleted, "할 일 Off -> On 작업에 실패했습니다")
        
        viewModel.onOffToDo(id: todoDummyDataID)
        XCTAssertFalse(viewModel.sortedTodo[0].isCompleted, "할 일 On -> Off 작업에 실패했습니다")
    }
    ...
    func test_deleteToDo() async {
        ...
        XCTAssertEqual(viewModel.sortedTodo.count, 1, "할 일이 삭제되지 않았습니다")
    }
}
